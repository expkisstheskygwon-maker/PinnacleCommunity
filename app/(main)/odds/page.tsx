...
export default function OddsPage() {
  ...
  const handleOpenMarkets = async (match: any) => {
    ...
    try {
      const res = await fetch(`/api/sports/markets?fixtureId=${match.id}&sport=${activeCat}`);
      const data = await res.json();
                            ...
                            {groupMatches.map(m => {
                              ...
                              return (
                                <React.Fragment key={m.id}>
                                  ...
                                  {isExpanded && (
                                    <tr className="bg-white/[0.01] border-l-2 border-primary/40 animate-in slide-in-from-top-1 duration-200">
                                      <td colSpan={9} className="p-0">
                                        <div className="px-5 py-6 grid grid-cols-1 md:grid-cols-3 gap-8">
                                          ...
                                          <div className="space-y-4">
                                            ...
                                            <div className="bg-black/20 rounded-xl p-4 border border-white/5 space-y-3 text-[11px]">
                                              <div className="flex justify-between text-[10px] border-b border-white/5 pb-2 font-bold text-muted-foreground uppercase">
                                                <span>시장</span>
                                                <span>초기</span>
                                                <span>현재</span>
                                              </div>
                                              <div className="flex justify-between"><span>홈 (1)</span><span className="text-muted-foreground">{(m.odds.h * 1.02).toFixed(2)}</span><span className="font-bold">{m.odds.h.toFixed(2)}</span></div>
                                              <div className="flex justify-between"><span>패 (2)</span><span className="text-muted-foreground">{(m.odds.a * 0.97).toFixed(2)}</span><span className="font-bold">{m.odds.a.toFixed(2)}</span></div>
                                            </div>
                                          </div>
                                          <div className="space-y-4">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4">
          ...
          <div className="relative w-full max-w-4xl glass-card rounded-[32px] overflow-hidden flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300">
            <div className="px-8 py-6 border-b border-white/[0.06] bg-white/[0.02] flex items-center justify-between">
              ...
              <button onClick={() => setSelectedMatch(null)} className="p-3 bg-white/5 hover:bg-white/10 rounded-2xl"><X className="w-6 h-6" /></button>
            </div>
            <div className="flex-1 overflow-y-auto p-8 bg-black/20">
              {marketLoading ? (
                <div className="flex flex-col items-center justify-center py-20"><div className="w-10 h-10 border-4 border-primary/30 border-t-primary rounded-full animate-spin" /></div>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {markets.map((market, idx) => (
                    <div key={idx} className="glass-card rounded-[24px] overflow-hidden border border-white/[0.04]">
                      <div className="px-5 py-3 border-b border-white/[0.04] bg-white/[0.02] font-black text-[10px] text-muted-foreground uppercase">{market.name}</div>
                      <div className="p-5 grid grid-cols-2 gap-2.5">
                        {market.values.map((v: any, vIdx: number) => (
                          <div key={vIdx} className="flex items-center justify-between px-4 py-3 bg-white/5 rounded-2xl border border-white/[0.04]">
                            <span className="text-[11px] font-bold text-muted-foreground">{v.value}</span>
                            <span className="text-sm font-black font-mono text-primary">{v.odd}</span>
                          </div>
                        ))}
                      </div>
                    </div>
            ...
            <div className="px-8 py-5 border-t border-white/[0.06] flex items-center justify-between">
              <span className="text-[10px] text-muted-foreground/60 uppercase">배당 실시간 연동 중 • Pinnacle Official</span>
              <button onClick={() => setSelectedMatch(null)} className="btn-primary py-2.5 px-8 text-xs h-auto">닫기</button>
            </div>
          </div>
        </div>
      )}
